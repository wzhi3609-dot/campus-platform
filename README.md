...
| 构建工具 | Vite |
| UI 库 | Element Plus（按需引入，茶道和风主题定制） |
| 状态管理 | Pinia |
| HTTP 客户端 | Axios |
| 后端构建 | Maven |
...
### v2.2.0 — 茶道和风视觉改版
- 前端全局切换为日式茶道和风（Wabi-Sabi）主题
- 页背景改为土陶白 `#f5f0e8`，主色改为抹茶绿 `#5c6e3d`
- 卡片/边框统一使用茶染色 `#e0d5c0`，暖棕阴影替代纯黑阴影
- 导航栏 Logo 改为圆形抹茶绿底 + 衬线体「互」字
- 首页 Hero 区改为抹茶绿渐变，模块图标圆形化，标题改用衬线字体
- 各模块主色暗化降饱和适配和风（蓝铁/苔绿/栗皮/红梅）
- 登录/注册页卡片圆角统一 16px，分段标签改为暖白色底
- 列表/详情页卡片圆角统一 14px，边框暖棕，文字色暖调
- Element Plus 全局 CSS 变量覆盖（按钮/输入框/分页/菜单等）
- 管理后台样式不受影响，保持原样
