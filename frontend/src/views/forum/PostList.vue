  ...
  <div>
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 12px;">
      <div>
        ...
        <p style="margin: 0; color: #909399; font-size: 13px;">自由交流，分享校园生活（发帖需审核）</p>
      </div>
    ...
    <div v-loading="loading">
      <div v-if="posts.length === 0 && !loading" style="text-align: center; padding: 60px 0; color: #909399;">
        <div style="font-size: 40px; margin-bottom: 12px;">📝</div>
        <p style="font-size: 15px;">暂无帖子</p>
        <el-button type="primary" plain @click="router.push('/forum/create')">发布第一个帖子</el-button>
      </div>
      <div v-for="post in posts" :key="post.id" style="background: #fff; border-radius: 10px; margin-bottom: 12px; cursor: pointer; border: 1px solid #f0f0f0; transition: all 0.2s; padding: 20px;" @click="router.push(`/forum/${post.id}`)" @mouseenter="$event.currentTarget.style.borderColor='#f9ab00'; $event.currentTarget.style.boxShadow='0 2px 8px rgba(249,171,0,0.08)'" @mouseleave="$event.currentTarget.style.borderColor='#f0f0f0'; $event.currentTarget.style.boxShadow='none'">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 12px;">
          <div style="flex: 1; min-width: 0;">
            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
              <el-tag v-if="post.pinned" type="danger" size="small" style="border-radius: 4px;">置顶</el-tag>
              <span style="font-size: 16px; font-weight: 500; color: #303133;">{{ post.title }}</span>
            </div>
            <p style="color: #909399; font-size: 13px; margin: 8px 0 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
          ...
          <div style="text-align: center; min-width: 48px;">
            <div style="font-size: 16px; font-weight: 600; color: #f9ab00;">{{ post.likeCount || 0 }}</div>
            <div style="font-size: 11px; color: #909399;">赞</div>
          </div>
        </div>
      </div>
    </div>
